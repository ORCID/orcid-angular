...
export class RecordHolderRolesPipe implements PipeTransform {
  ...
  transform(contributors: Contributor[], isPublicRecord: string, id: string): string {
    let roles = null
    if (contributors?.length > 0) {
      contributors.find(contributor => {
        return isPublicRecord ? contributor?.contributorOrcid?.path === isPublicRecord : contributor?.contributorOrcid?.path === id
