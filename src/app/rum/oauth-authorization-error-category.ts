...
export function getOauthAuthorizationErrorCategory(
  ...
  const checks: Array<{ match: (s: string) => boolean; category: string }> = [
    ...
    { match: (s) => s.includes('invalid_request'), category: 'invalid_request' },
    {
    ...
    { match: (s) => s.includes('unauthorized_client'), category: 'unauthorized_client' },
    { match: (s) => s.includes('client_locked'), category: 'client_locked' },
    { match: (s) => s.includes('interaction_required'), category: 'interaction_required' },
