...
function normalizeOrcidId(rawValue) {
    if (!rawValue) return "";
    const normalized = rawValue.trim().replace(/^https?:\/\/(www\.)?orcid\.org\//i, "");
...
function sanitizeUrl(rawUrl) {
    ...
    try {
        ...
        if ((host === "orcid.org" || host === "www.orcid.org") && /^\/client\//i.test(path)) {
            return "";
        }
...
function resolveOrcidIdFromLocation() {
    ...
    if (pathSegments.length >= 2) {
        ...
        if (last === "print" || last === "print-view") {
            const fromPrintPath = normalizeOrcidId(pathSegments[pathSegments.length - 2]);
...
function renderOrcidPrompt(message = "") {
    ...
    form.addEventListener("submit", (event) => {
        ...
        if (!orcidId) {
            formError.textContent = "Enter a valid ORCID iD (format: 0000-0000-0000-0000).";
            input.focus();
            return;
        }
        clearStatus();
...
function jsonText(value) {
    ...
    if (typeof value === "object") {
        ...
        if (value.value !== null && value.value !== undefined) return String(value.value).trim();
    }
    return "";
}
...
function renderPersonalInfoFromJson(recordJson, container) {
    ...
    if (emails.length) {
        ...
        appendList(block, emails.map((email) => textLineNode("", email)));
        blocks.push(block);
    }
    ...
    if (links.length) {
        ...
        const lines = links.map((entry) => {
            ...
            row.className = "line";
            if (entry.name) row.appendChild(document.createTextNode(`${entry.name} (`));
...
function composeActivityEntryFromJson(entry, opts = {}) {
    ...
    const title = opts.title || jsonText(entry?.title?.title) || jsonText(entry?.organization?.name) || jsonText(entry?.roleTitle) || jsonText(entry?.type) || "Untitled";
    const titleNode = document.createElement("strong");
    titleNode.textContent = title;
    wrapper.appendChild(titleNode);

    const org = jsonText(entry?.organization?.name);
    if (org && org !== title) wrapper.appendChild(textLineNode("Organization", org));

    const orgAddress = [jsonText(entry?.organization?.address?.city), jsonText(entry?.organization?.address?.region), jsonText(entry?.organization?.address?.country)].filter(Boolean).join(", ");
    if (orgAddress) wrapper.appendChild(textLineNode("Organization address", orgAddress));
    ...
    if (Array.isArray(extIds) && extIds.length) {
        extIds.forEach((id) => {
            ...
            const idUrl = sanitizeUrl(jsonText(id.url?.value || id.externalIdUrl?.value || id.externalIdUrl || id.url));
            const label = (idRel && idRel.toLowerCase() !== "self") ? `${idRel} ${idType}` : (idType || "Identifier");
...
function renderActivitiesFromJson(recordJson, container) {
    ...
    const workSummaries = (activities?.works?.workGroup || []).flatMap((g) => g?.workSummary || []);
    hasContent = renderActivityGroupFromJson(section, "Works", workSummaries, (w) =>
    ...
    const educations = (activities?.educations?.educationGroups || []).flatMap((g) => g?.activities || []);
    hasContent = renderActivityGroupFromJson(section, "Educations", educations, (e) =>
        composeActivityEntryFromJson(e, { title: jsonText(e?.organization?.name) || jsonText(e?.roleTitle) })
    ) || hasContent;

    // Employments: activitiesSummary.employments.employmentGroups[].activities[]
    const employments = (activities?.employments?.employmentGroups || []).flatMap((g) => g?.activities || []);
    hasContent = renderActivityGroupFromJson(section, "Employments", employments, (e) =>
        composeActivityEntryFromJson(e, { title: jsonText(e?.organization?.name) || jsonText(e?.roleTitle) })
    ) || hasContent;

    // Fundings: activitiesSummary.fundings.fundingGroup[].activities[]
    const fundings = (activities?.fundings?.fundingGroup || []).flatMap((g) => g?.activities || []);
    hasContent = renderActivityGroupFromJson(section, "Fundings", fundings, (f) =>
        composeActivityEntryFromJson(f, { title: jsonText(f?.title?.title) || jsonText(f?.type) })
    ) || hasContent;

    // Peer reviews: activitiesSummary.peerReviews.peerReviewGroup[].peerReviewGroup[].peerReviewSummary[]
    const peerReviews = (activities?.peerReviews?.peerReviewGroup || []).flatMap((g) =>
        (g?.peerReviewGroup || []).flatMap((gg) => gg?.peerReviewSummary || [])
    );
    hasContent = renderActivityGroupFromJson(section, "Peer reviews", peerReviews, (p) =>
        composeActivityEntryFromJson(p, { title: jsonText(p?.organization?.name) || jsonText(p?.groupId) })
    ) || hasContent;

    // Qualifications: activitiesSummary.qualifications.qualificationGroups[].activities[]
    const qualifications = (activities?.qualifications?.qualificationGroups || []).flatMap((g) => g?.activities || []);
    hasContent = renderActivityGroupFromJson(section, "Qualifications", qualifications, (q) =>
        composeActivityEntryFromJson(q, { title: jsonText(q?.organization?.name) || jsonText(q?.roleTitle) })
    ) || hasContent;

    // Research resources: activitiesSummary.researchResources.researchResourceGroup[].activities[] (or researchResourceSummary)
    const researchResources = (activities?.researchResources?.researchResourceGroup || []).flatMap((g) => g?.activities || []);
    hasContent = renderActivityGroupFromJson(section, "Research resources", researchResources, (r) =>
...
async function fetchOrcidRecord(orcidId) {
    ...
    const url = new URL(`/${encodeURIComponent(orcidId)}/record`, window.location.origin);
    ...
    if (response.status === 301 || response.status === 302) {
        const primaryFromHeader = normalizeOrcidId(response.headers.get("x-orcid-primary"));
