...
describe('Primary account email verification reminders', async function() {
    ...
    it('Receive reminder email if primary account email has not been verified', function() { 
        ...
        cy.log('Signing in with user: '+ userData.cyUser_primaryEmailNotVerified.oid)
        cy.get('#username').clear().type(userData.cyUser_primaryEmailNotVerified.oid)
        cy.get('#password').clear().type(userData.cyUser_primaryEmailNotVerified.password)
        cy.get('#signin-button').click()

        //request to get reminder email to verify primary account email
        
        cy.get('[class="row resend-verification"]').get('[mat-raised-button]').contains('Resend').click()
        ...
        cy.task("readAllMessages", {
           ...
           // cy.log(htmlDom.documentElement.innerHTML)
            ...
            cy.get(htmlDom.querySelector('[id$="verificationButton"]')).invoke('attr','href').should('include', Cypress.env('verifyEmailUrl'))
        })
