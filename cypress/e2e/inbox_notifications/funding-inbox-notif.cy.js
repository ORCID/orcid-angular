...
describe('Inbox: add/update/delete funding via API', async function () {
  ...
  "' -d '" +
  userData.cyNotifPerm.curlFundingPath + "' -X POST '"+
  ...
  Cypress.env('membersAPI_URL') +
  userData.cyNotifPerm.oid +
  Cypress.env('membersAPI_fundingsEndpoint')+ "/"//here append "{PUTCODE}" + "'"
   ...
   //Client adds an work
    ...
    cy.exec(curlReadAllFundings).then((response) => {
      ...
      cy.exec(curlReadSingleFunding+putCode+"'").then((singleFundResp => {
        ...
        updatedContent=updatedContent.replace(userData.cyNotifPerm.fundingToBeReplaced,userData.cyNotifPerm.fundingReplaceWith)
        //write the file to use for the PUT
        cy.writeFile(userData.cyNotifPerm.funding_putWriteFilePath, updatedContent)
      }))
