...
describe('Inbox: add/update/delete Employment via API', async function () {
  ...
  "' -d '" +
  userData.cyNotifPerm.curlEmploymentPath + "' -X POST '"+
  ...
  Cypress.env('membersAPI_URL') +
  userData.cyNotifPerm.oid +
  Cypress.env('membersAPI_employmentEndpoint')+ "/"//here append "{PUTCODE}" + "'"
  ...
  it('Inbox notifications are received when Employment is added/updated/deleted via API', function () {
   ...
   //Client adds an employment
    ...
    cy.exec(curlReadAllEmployments).then((response) => {
      ...
      putCode = responseString.substring(putCodeStartPosition, putCodeEndPosition)
      cy.exec(curlReadSingleEmployment+putCode+"'").then((singleEmploymentResp => {
        //remove non json header from string
        const jsonStartIndex=singleEmploymentResp.stdout.indexOf('{"created-date"')//where does the json start?
        updatedContent = singleEmploymentResp.stdout.substring(jsonStartIndex)
        //update the employment: make a change in the content
        updatedContent=updatedContent.replace(userData.cyNotifPerm.contentToBeReplaced,userData.cyNotifPerm.replaceWith)
