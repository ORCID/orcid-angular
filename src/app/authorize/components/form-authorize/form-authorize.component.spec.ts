...
describe('FormAuthorizeComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    const userServiceSpy = jasmine.createSpyObj('UserService', ['noRedirectLogout', 'getUserSession'])
    const togglzServiceSpy = jasmine.createSpyObj('TogglzService', ['getStateOf'])
    const platformInfoServiceSpy = jasmine.createSpyObj('PlatformInfoService', ['get'])
    const trustedIndividualsServiceSpy = jasmine.createSpyObj('TrustedIndividualsService', ['getTrustedIndividuals'])
    const oauthServiceSpy = jasmine.createSpyObj('OauthService', ['authorize', 'authorizeOnAuthServer'])
    const snackbarServiceSpy = jasmine.createSpyObj('SnackbarService', ['showError', 'showSuccess'])
    const errorHandlerServiceSpy = jasmine.createSpyObj('ErrorHandlerService', ['handleError'])
    const oauthURLSessionManagerServiceSpy = jasmine.createSpyObj('OauthURLSessionManagerService', ['getOauthUrlSession'])
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: TrustedIndividualsService, useValue: trustedIndividualsServiceSpy },
        { provide: OauthService, useValue: oauthServiceSpy },
        { provide: SnackbarService, useValue: snackbarServiceSpy },
        { provide: ErrorHandlerService, useValue: errorHandlerServiceSpy },
        { provide: OauthURLSessionManagerService, useValue: oauthURLSessionManagerServiceSpy },
  ...
  beforeEach(() => {
    ...
    platformInfoService = TestBed.inject(PlatformInfoService) as jasmine.SpyObj<PlatformInfoService>
    mockWindow = TestBed.inject(WINDOW) as jasmine.SpyObj<Window>
    
    // Setup default mocks
    togglzService.getStateOf.and.returnValue(of(false))
    platformInfoService.get.and.returnValue(of({
      rtl: false,
      ltr: true,
    ...
    userService.getUserSession.and.returnValue(of({
    ...
    const trustedIndividualsService = TestBed.inject(TrustedIndividualsService) as jasmine.SpyObj<TrustedIndividualsService>
    trustedIndividualsService.getTrustedIndividuals.and.returnValue(of({
      delegators: []
  ...
  describe('logout', () => {
    ...
    it('should redirect to signin with query parameters on successful logout', () => {
      ...
      userService.noRedirectLogout.and.returnValue(of(new HttpResponse({ body: 'OK' })))

      component.logout()

      expect(userService.noRedirectLogout).toHaveBeenCalled()
      expect(mockWindow.location.href).toBe('/signin?email=test%40example.com&scope=read+write&state=test-state')
    })
    ...
    it('should handle special characters in email query parameter', () => {
      ...
      expect(mockWindow.location.href).toBe('/signin?email=test%2Bspecial%40example.com&scope=read+write&state=test-state')
    })
    ...
    it('should handle email with unicode characters', () => {
      ...
      expect(mockWindow.location.href).toBe('/signin?email=t%C3%ABst%40%C3%A9x%C3%A4mpl%C3%A9.com&scope=read+write&state=test-state')
    })
    ...
    it('should handle spaces in scope parameter correctly', () => {
      ...
      expect(mockWindow.location.href).toBe('/signin?email=test%40example.com&scope=read+write+update&state=test-state')
    })
    ...
    it('should handle complex query parameters with special characters', () => {
      ...
      expect(mockWindow.location.href).toContain('email=user%2Btag%40sub.domain.com')
    ...
    it('should redirect to signin even when noRedirectLogout fails', (done) => {
      ...
      component.platformInfo.queryParameters = queryParams
      userService.noRedirectLogout.and.returnValue(throwError(() => new Error('Logout failed')))
      ...
      setTimeout(() => {
        expect(userService.noRedirectLogout).toHaveBeenCalled()
        expect(mockWindow.location.href).toBe('/signin?email=test%40example.com&scope=read+write')
        done()
      }, 100)
    })
