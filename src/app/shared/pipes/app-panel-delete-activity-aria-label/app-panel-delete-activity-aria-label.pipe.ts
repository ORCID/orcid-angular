...
@Pipe({
  name: 'appPanelDeleteActivityAriaLabel',
})
export class AppPanelDeleteActivityAriaLabelPipe implements PipeTransform {
  transform(panelType: string, title: string): string {
    let translationWithPlaceholders = ($localize`:@@shared.deleteActivityAriaLabel:Delete %TYPE% %TITLE%`).replace(PANEL_TITLE_PLACEHOLDER, title)
    if (panelType == 'employment' ){
      ...
      return translationWithPlaceholders.replace(PANEL_TYPE_PLACEHOLDER, $localize`:@@shared.worksAriaLabel:work`)
    }
