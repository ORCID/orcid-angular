...
export class GoogleTagManagerService {
  ...
  public addGtmToDom(): Observable<boolean> {
    return new Observable<boolean>((subscriber) => {
      ...
      gtmScript.src = 'https://www.googletagmanager.com/gtm.js?id=' + environment.GOOGLE_TAG_MANAGER
  ...
  reportNavigationEnd(url: string): Observable<void> {
    const duration = finishPerformanceMeasurement(url, this.browserGlobals.windowRef())
    if (duration) {
      if (environment.debugger) {
