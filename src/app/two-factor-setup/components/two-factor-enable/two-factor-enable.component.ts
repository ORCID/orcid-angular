...
@Component({
  ...
  providers: [{provide: MAT_TOOLTIP_DEFAULT_OPTIONS, useValue: clipboardTooltip}],
    ...
    this._twoFactorService
      ...
      .subscribe((res) => {
        ...
        if (!res.valid) {
          ...
          this.twoFactorEnabled.emit({backupCodes, backupCodesClipboard: res.backupCodes.join(' ')  })
        }
      })
  }

  textCode() {
    this._twoFactorService.getTextCode()
      .pipe(first())
      .subscribe((result) => {
        this.showTextCode = true
        this.twoFactorForm.addControl('textCode', new FormControl(
          { value: result.secret, disabled: true },
          {}))
      })
  }
