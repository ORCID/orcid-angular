...
export class WorkFormComponent implements OnInit {
  ...
  private getContributorRoles(): Contributor[] {
    ...
    const roles = rolesFormArray?.controls
      ?.filter((fg) => fg?.value?.role && (this._workService.getContributionRoleByKey(fg?.value?.role) !== null && this._workService.getContributionRoleByKey(fg?.value?.role).key !== 'no specified role'))
      .map((formGroup) => {
