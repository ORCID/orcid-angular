...
<Meta title="Docs|Theaming" decorators={[moduleMetadata({ imports: [] })]} />
...
⚠️After declaring this mixing it must be imported and declared on `src/assets/scss/material.orcid-components.scss` to be declared globally for the application.
...
⚠️It's recomended to check what other elements are affect when a color is overwrited on the Orcid App and on the Angular Material library.
