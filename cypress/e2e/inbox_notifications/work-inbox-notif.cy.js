...
describe('Inbox: add/update/delete work via API', async function () {

  const curlAddWork =
  "curl -i -H 'Content-type: application/json' -H 'Authorization: Bearer "+
  userData.cyNotifPerm.clientBearer+
  "' -d '" +
  userData.cyNotifPerm.curlWorkPath + "' -X POST '"+
  ...
  Cypress.env('membersAPI_URL') +
  userData.cyNotifPerm.oid +
  Cypress.env('membersAPI_workEndpoint')+ "/"//here append "{PUTCODE}" + "'"
 ...
 "curl -i -H 'Content-type: application/json' -H 'Authorization: Bearer "+
  ...
  it('Inbox notifications are received when Work is added/updated/deleted via API', function () {
   let putCode
   let updatedContent
   ...
   //Client adds an work
    ...
    cy.exec(curlReadAllWorks).then((response) => {
      ...
      cy.exec(curlReadSingleWork+putCode+"'").then((singleWorkResp => {
        ...
        updatedContent=updatedContent.replace(userData.cyNotifPerm.workToBeReplaced,userData.cyNotifPerm.workReplaceWith)
