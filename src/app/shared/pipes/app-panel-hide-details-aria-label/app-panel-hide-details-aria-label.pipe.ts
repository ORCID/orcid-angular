...
@Pipe({
  name: 'appPanelHideDetailsAriaLabel',
})
export class AppPanelHideDetailsAriaLabelPipe implements PipeTransform {
  transform(panelType: string, title: string): string {
    console.log("panel type is: " + panelType)
    let translationWithPlaceholders = ($localize`:@@shared.activityHideDetailsAriaLabel:Hide details for %TYPE% %TITLE%`).replace(PANEL_TITLE_PLACEHOLDER, title)
