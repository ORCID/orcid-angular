...
describe("Password reset and OID recovery", () => {
    ...
    it.skip("User resets password", function() {
     const newPassword = "12345678Ab";
        ...
        //type email
        cy.get('[formcontrolname="email"]').clear().type(userData.cyUser_primaryEmaiVerified.email)
        ...
        cy.task("readAllMessages", {
            ...
            const href= htmlDom.querySelector('[href^=Cypress.env("resetPasswEmailURL")]').href
    ...
    it("User recovers OID", function() {
        ...
        cy.task("readAllMessages", {
           ...
           const href= htmlDom.querySelector(`a[href*='${userData.cyUser_primaryEmaiVerified.oid}']`).href    
            cy.log(">>>>>>>found the link: "+ href)
            //follow the link from the email
            cy.visit(href)
        })
