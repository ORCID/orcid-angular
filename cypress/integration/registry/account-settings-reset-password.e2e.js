...
describe('Account Settings - users can reset their password', async function () {
    ...
    it('Verify user is able to reset the password to a different valid password and log in', function () {
     ...
     cy.get('[formcontrolname="oldPassword"]').clear().type(userData.cyAcctSettRestPasswUser.password)
     cy.get('[formcontrolname="password"]').clear().type(userData.cyAcctSettRestPasswUser.newPassw)
     cy.get('[formcontrolname="retypedPassword"]').clear().type(userData.cyAcctSettRestPasswUser.newPassw)
    ...
    cy.get('[formcontrolname="oldPassword"]').clear().type(userData.cyAcctSettRestPasswUser.password)
    cy.get('[formcontrolname="password"]').clear().type(userData.cyAcctSettRestPasswUser.password)
    cy.get('[formcontrolname="retypedPassword"]').clear().type(userData.cyAcctSettRestPasswUser.password)
    cy.get('#cy-save-password').click()
    cy.contains('Your password has been updated').should('be.visible')//wait for update to complete
    //log out
    cy.get('#cy-user-info').click()
