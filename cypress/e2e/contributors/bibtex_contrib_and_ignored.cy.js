...
describe('Add work using BibTeX citation where contributor list ends with "and"', async function () {
    const bibtextFilePath = userData.cyRecordOwner.bibtex_qase73_path
    const contribName = userData.cyRecordOwner.bibtex_qase73_contrib
  ...
  qase(
    ...
    it('Add work using BibTeX citation where contributor list ends with "and"', function () {
     ...
     //Summary view - 'and' is ignored
     cy.contains('app-panel-data', contribName).within(
      ($thisWork) => {
        cy.contains('Contributors').parent().should('not.contain','and')
        cy.contains('Show more detail').click()
      }
    )  
    //Details section - 'and' is ignored
    cy.contains('app-display-attribute','Contributors').should('not.contain','and')
    })
